...
# MAGIC %md
# MAGIC # Tuning Tables for Performance
# MAGIC </br>
...
# MAGIC </br>
# MAGIC Use the latest LTS Databricks Runtime to take advantage of the latest performance tuning features
...
# DANGER: BAD PRACTICES IN THIS CELL FOR TUNING DEMO PURPOSES ONLY. DO NOT REPLICATE!!! 
def test_etl(target):
  ''' 
  ...
  for i in range(3):
    ...
    ''')
  iot_data_generator(spark, 1000000).write.mode('append').option('maxRecordsPerFile', '1000').saveAsTable(target)
# !!! IMPORTANT - BAD PRACTICES IN THIS CELL FOR TUNING DEMO PURPOSES ONLY. DO NOT REPLICATE!!!
...
''')
spark.sql(f''' -- WARNING: By turning off deletion vectors we slow down merges, updates, deletes, and "needle in the haystack" lookups
  ALTER TABLE {config['tuned_bronze_table']} SET TBLPROPERTIES ('delta.enableDeletionVectors' = false);
''')
...
display(spark.sql(f"DESCRIBE DETAIL {config['tuned_bronze_table']}").select("numFiles"))
...
display(spark.sql(f"DESCRIBE DETAIL {config['tuned_bronze_table']}").select("numFiles")) # ... way fewer files!
