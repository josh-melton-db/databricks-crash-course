...
# MAGIC %md
# MAGIC # End-to-End ML Lifecycle Management
# MAGIC
# MAGIC # MLflow and Production AI/ML
# MAGIC
# MAGIC End-to-end machine learning lifecycle management with MLflow 3 on Databricks.
# MAGIC
# MAGIC ## What You'll Learn
# MAGIC
# MAGIC ✅ Track experiments with MLflow 3  
# MAGIC ✅ Register models in Unity Catalog  
# MAGIC ✅ Deploy models to production  
# MAGIC ✅ Monitor model performance  
# MAGIC ✅ Implement MLOps best practices  
# MAGIC
# MAGIC **Key Concepts:**
# MAGIC - **MLflow 3**: Latest version with Logged Models and Deployment Jobs
# MAGIC - **Experiment Tracking**: Log parameters, metrics, and artifacts
# MAGIC - **Model Registry**: Version control for models in Unity Catalog
# MAGIC - **Model Serving**: Deploy models as REST APIs
# MAGIC - **Monitoring**: Track predictions and drift
# MAGIC
# MAGIC ---
# MAGIC
# MAGIC **References:**
# MAGIC - [MLflow Documentation](https://docs.databricks.com/aws/en/mlflow/)
# MAGIC - [MLflow 3 Install](https://docs.databricks.com/aws/en/mlflow/mlflow-3-install)
# MAGIC - [ML Workflow](https://docs.databricks.com/aws/en/mlflow/mlflow3-ml-workflow)
# MAGIC - [Model Lifecycle](https://docs.databricks.com/aws/en/machine-learning/manage-model-lifecycle/)
# MAGIC - [Model Tracking](https://docs.databricks.com/aws/en/mlflow/tracking)
...
df = spark.read.table(f'{catalog}.{schema}.airbnb').toPandas()
X_train, X_test, y_train, y_test = train_test_split(pd.get_dummies(df.drop(["price"], axis=1)), df["price"], random_state=42)
display(X_train)
...
with mlflow.start_run(run_name="Basic RF Run") as run:
    ...
    # Log model
    mlflow.sklearn.log_model(rf, model_name, input_example=X_train[:5], signature=signature)
...
# MAGIC %md
# MAGIC Key tip: just use `mlflow.autolog()`! This example illustrates how MLflow works, but you can dramatically reduce the code required by using autolog - more information [here](https://docs.databricks.com/aws/en/mlflow/databricks-autologging)
...
# MAGIC
# MAGIC You can also set tags for runs using `client.set_tag(run.info.run_id, "tag_key", "tag_value")`
...
# MAGIC This is where we jump into Unity Catalog - if you don't have Unity Catalog, you can follow the same pattern with the [workspace model registry](https://docs.databricks.com/en/machine-learning/manage-model-lifecycle/workspace-model-registry.html)

# COMMAND ----------

mlflow.set_registry_uri("databricks-uc")
model_uri = f"runs:/{run_rf.run_id}/{model_name}"
model_details = mlflow.register_model(model_uri=model_uri, name=f"{catalog}.{schema}.{model_name}")
...
client.update_registered_model(
  ...
  description="This model predicts the price of an airbnb"
)

client.set_registered_model_alias(f"{catalog}.{schema}.{model_name}", "Champion", model_details.version)

# COMMAND ----------

import mlflow.pyfunc

model_champion_uri = "models:/{model_name}@Champion".format(model_name=f"{catalog}.{schema}.{model_name}")
...
predictions = pd.DataFrame(champion_model.predict(X_train))
predictions
